...
const DashboardCard = ({ title, value, change, isPositive }) => {
  return (
    ...
    >
      <Typography variant="body2" sx={{ color: "#9ca3af", marginBottom: "0.25rem" }}>
        {title}
      </Typography>
      <Typography variant="h5" sx={{ fontWeight: "bold", color: "#f3f4f6" }}>
        {value}
      </Typography>
      <Box sx={{ display: "flex", alignItems: "center", fontSize: "0.75rem", color: "#9ca3af" }}>
        <ArrowUpRight 
          style={{ 
            height: "0.75rem", 
            width: "0.75rem", 
...
const ChartCard = ({ title, children }) => {
  return (
    ...
    >
      ...
      <Box sx={{ padding: "0.75rem", flexGrow: 1 }}>
        {children}
      </Box>
    </Box>
  );
